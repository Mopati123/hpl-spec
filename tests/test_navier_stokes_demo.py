...
class NavierStokesDemoTests(unittest.TestCase):
    def test_navier_stokes_demo_deterministic(self):
        with tempfile.TemporaryDirectory() as tmp_one:
            ...
            result_one = _run_cmd(
                [
                    ...
                    str(out_one),
                    "--state",
                    str(FIXTURES / "ns_state_initial.json"),
                    "--policy",
                    str(FIXTURES / "ns_policy_safe.json"),
                    "--signing-key",
                    str(TEST_KEY),
                ]
            )
    ...
    def test_navier_stokes_demo_refusal(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = _run_cmd(
                [
                    ...
                    str(FIXTURES / "ns_state_initial.json"),
                    "--policy",
                    str(FIXTURES / "ns_policy_forbidden.json"),
                    "--signing-key",
                    str(TEST_KEY),
