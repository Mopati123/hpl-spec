...
class AgentGovernanceDemoTests(unittest.TestCase):
    def test_agent_governance_demo_deterministic(self):
        with tempfile.TemporaryDirectory() as tmp_dir_one:
            ...
            result_one = _run_cmd(
                [
                    ...
                    str(out_one),
                    "--proposal",
                    str(ROOT / "tests" / "fixtures" / "agent_proposal_allow.json"),
                    "--policy",
                    str(ROOT / "tests" / "fixtures" / "agent_policy.json"),
                    "--signing-key",
                    str(TEST_KEY),
                ]
            )
    ...
    def test_agent_governance_demo_refusal(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            result = _run_cmd(
                [
                    ...
                    str(out_dir),
                    "--proposal",
                    str(ROOT / "tests" / "fixtures" / "agent_proposal_deny.json"),
                    "--policy",
                    str(ROOT / "tests" / "fixtures" / "agent_policy.json"),
                    "--signing-key",
                    str(TEST_KEY),
