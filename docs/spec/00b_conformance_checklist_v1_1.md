# HPL Conformance Checklist — v1.1

## Purpose

This checklist certifies whether an implementation conforms to **HPL Spec v1.1** as declared in:
- `docs/spec/00_spec_freeze_declaration_v1_1.md` (to be issued)

Conformance is defined as: implementing behavior that is consistent with all **normative v1.1 documents** and does not introduce semantics, syntax, fields, or operator classes beyond the frozen scope.
...
Informational (not required for v1.1 conformance):
...
- **Level 3+ — Runtime/Backends:** Out of scope for v1.1 unless explicitly added by a new spec version.
...
[ ] L1.16 Operator class assignment conforms to v1.1 classification rules (registry-declared).
...
[ ] L2.4 Diagnostics emit stable, machine-readable error fields (per taxonomy).
...
[ ] F.1 Implementing simulator/runtime/scheduler/backends as normative behavior under v1.1 without a new spec version.
[ ] F.2 Allowing surface constructs to reach IR construction.
[ ] F.3 Adding new operator classes, IR fields, or grammar constructs not present in v1.1 specs.
[ ] F.4 Accepting unknown IR fields or silently ignoring them.
[ ] F.5 Defaulting all operators to `C` (bootstrap default removed in v1.1).
